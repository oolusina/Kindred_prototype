...
export default function PhoneFrame({ children }: { children: ReactNode }) {
  ...
  if (mobile) {
    return (
      <div className="min-h-dvh w-full overflow-clip bg-canvas">
        <div className="relative h-dvh w-full overflow-clip bg-canvas pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
          <div
          ...
          >
            <div className="flex min-h-0 flex-1 flex-col overflow-clip">{children}</div>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-dvh w-full bg-[#e9e5dd] sm:flex sm:items-center sm:justify-center sm:py-8">
      <div className="relative mx-auto flex h-dvh w-full min-h-0 flex-col overflow-clip bg-canvas sm:h-[844px] sm:w-[390px] sm:rounded-[54px] sm:shadow-[0_24px_80px_rgba(31,26,23,0.35)] sm:ring-8 sm:ring-[#1c1917]">
        <div className="flex min-h-0 flex-1 flex-col overflow-clip">{children}</div>
      </div>
